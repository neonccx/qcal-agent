...
class PhysicalSimulator:
    ...
    def __init__(self, seed=20260903, noise_scale=1., profile="nominal"):
        ...
        self._truth = {"fr": 6.5e9+self.rng.uniform(-8e6, 8e6),
            ...
            "readout_optimum_dbm": self.rng.uniform(-24., -16.),
            "resonator_power_shift_hz": self.rng.uniform(4e6, 9e6),
    ...
    def acquire(self, tool, state, scan):
        ...
        if tool in ("sq.s21", "sq.spectroscopy"):
            ...
            grid = []
            low_power_resonance = self._readout_frequency(state["z_bias"])
            for power in powers:
                saturation = 1/(1+np.exp(-(power-t["readout_optimum_dbm"]-5)/2.4))
                resonance = low_power_resonance+t["resonator_power_shift_hz"]*saturation
                depth = t["depth"]*(1-.35*saturation)
                trace = complex_notch(frequency, resonance, t["ql"], depth, t["mismatch"],
                    t["gain"], t["phase"], t["delay"], center)
                trace += noise(len(frequency))+1j*noise(len(frequency))
                grid.append(trace)
            grid = np.asarray(grid)
            obs["sweep"] = {"frequency_values_hz": frequency.tolist(),
                            "power_values_dbm": powers.tolist(),
            ...
            power_gain = np.exp(-((state["readout_power_dbm"]-t["readout_optimum_dbm"])/11.)**2)
            groups = []
            for population in (thermal, thermal+(1-2*thermal)*excitation):
                ...
                z *= power_gain*np.exp(1j*t["phase"])
