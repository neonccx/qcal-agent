    ...
    def decide(self, context: dict) -> dict:
        ...
        if not obs["quality"]["reliable"]:
            ...
            if tool in ("sq.s21", "sq.spectroscopy"):
                ...
                scan["power_points"] = min(81, max(31, scan.get("power_points", 31)+20))
        ...
        if tool in ("sq.s21", "sq.spectroscopy"):
            ...
            updates = {"readout_power_dbm": fit["readout_power_dbm"],
                       "readout_frequency_hz": fit["readout_frequency_hz"]}
        elif tool == "sq.s21_zpa2d":
...
SYSTEM_PROMPT = """You are a single-qubit calibration policy running ONLY a registered calibration backend.
...
Runtime schema 1.0 uses this native action schema. Do not output diagnosis,
...
Tools must respect prerequisites in order: s21, s21_power2d, s21_zpa2d, spectroscopy, piamp, ramsey_df,
...
S21_POWER2D scans readout frequency against dBm power and selects the highest stable
pre-saturation operating point. Apply both fitted readout_power_dbm and readout_frequency_hz.
...
class HuggingFacePolicy:
    ...
    def decide(self, context: dict) -> str:
        ...
        if enabled:
            self.last_generation = dict(self.last_generation or {}, protocol="calibration-step-fit-1.0", native_call=raw)
