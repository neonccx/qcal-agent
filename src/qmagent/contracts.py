...
TOOLS = ("sq.s21", "sq.s21_power2d", "sq.s21_zpa2d", "sq.spectroscopy", "sq.piamp", "sq.ramsey_df",
...
DEFAULT_STATE = {
    "readout_frequency_hz": 6.5e9, "readout_power_dbm": -30.0,
    "z_bias": 0.0, "drive_frequency_hz": 5e9,
...
SCAN_LIMITS = {
    ...
    "power_min_dbm": (-70.0, -5.0), "power_max_dbm": (-50.0, 5.0),
    "power_points": (11, 81),
...
SCAN_KEYS = {
    ...
    "sq.s21_power2d": {"frequency_center_hz", "frequency_span_hz", "frequency_points",
                         "power_min_dbm", "power_max_dbm", "power_points"},
...
def parse_decision(value: Any) -> dict:
    ...
    if not isinstance(value, dict) or set(value) != {"next_tool", "parameter_action", "reason"}:
        raise ContractError("Expected exactly next_tool, parameter_action, reason (runtime schema 1.0)")
    ...
    for key in ("shots", "zpa_points", "frequency_points", "power_points", "max_depth", "depth_points", "circuits_per_depth"):
...
def validate_scan(tool: str, state: dict, scan: dict) -> None:
    ...
    for key in ("shots", "zpa_points", "frequency_points", "power_points", "max_depth", "depth_points", "circuits_per_depth"):
        if key in scan and type(scan[key]) is not int:
            raise ContractError(f"{key} must be an integer")
    if tool in ("sq.s21", "sq.s21_power2d", "sq.s21_zpa2d", "sq.spectroscopy"):
    ...
    if tool == "sq.s21_power2d":
        minimum, maximum = scan.get("power_min_dbm", -45.0), scan.get("power_max_dbm", -5.0)
        if minimum >= maximum:
            raise ContractError("Power2D minimum power must be below maximum power")
        if scan.get("power_points", 31) < 11 or scan.get("frequency_points", 241) < 101:
            raise ContractError("Power2D grid is undersampled")
...
def validate_observation(observation: dict, tool: str, candidate: dict) -> None:
    ...
    if tool in ("sq.s21_power2d", "sq.s21_zpa2d"):
        ...
        frequencies = sweep.get("frequency_values_hz")
        rows = sweep.get("power_values_dbm" if tool == "sq.s21_power2d" else "zpa_values")
        minimum_rows = 11 if tool == "sq.s21_power2d" else 21
        if (len(i_shape) != 2 or not isinstance(frequencies, list) or not isinstance(rows, list)
                or i_shape != (len(rows), len(frequencies)) or len(rows) < minimum_rows or len(frequencies) < 101):
            raise ContractError(f"{tool} axes do not match the I/Q grid")
        if any(b <= a for axis in (frequencies, rows) for a, b in zip(axis, axis[1:])):
            raise ContractError(f"{tool} axes must be strictly increasing")
    elif len(i_shape) != 1:
        raise ContractError("Only registered 2D S21 experiments may return two-dimensional I/Q arrays")
    required = {"sq.s21": ("frequency_hz",),
                "sq.s21_power2d": ("readout_power_dbm", "readout_frequency_hz"),
