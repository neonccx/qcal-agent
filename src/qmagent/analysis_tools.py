...
def fit_s21_power2d(obs):
    """Choose the highest stable readout power before resonator saturation.

    Only the measured complex grid is used. The selected point must retain notch
    contrast and remain on the low-power resonance plateau.
    """
    frequency = np.asarray(obs["sweep"]["frequency_values_hz"], dtype=float)
    powers = np.asarray(obs["sweep"]["power_values_dbm"], dtype=float)
    z = np.asarray(obs["measurement"]["i"], dtype=float)+1j*np.asarray(obs["measurement"]["q"], dtype=float)
    ridge, contrast = [], []
    for trace in z:
        magnitude = np.abs(trace)
        smooth = savgol_filter(magnitude, min(9, len(trace) if len(trace) % 2 else len(trace)-1), 2)
        index = int(np.argmin(smooth))
        if 0 < index < len(frequency)-1:
            local_x = frequency[index-1:index+2]-frequency[index]
            coefficients = np.polyfit(local_x, smooth[index-1:index+2], 2)
            offset = -coefficients[1]/(2*coefficients[0]) if coefficients[0] > 0 else 0.
            ridge.append(float(frequency[index]+np.clip(offset, local_x[0], local_x[-1])))
        else:
            ridge.append(float(frequency[index]))
        edge = max(8, len(magnitude)//10)
        baseline = float(np.median(np.r_[magnitude[:edge], magnitude[-edge:]]))
        contrast.append((baseline-float(np.min(smooth)))/max(baseline, 1e-12))
    ridge, contrast = np.asarray(ridge), np.asarray(contrast)
    reference_count = max(3, len(powers)//5)
    low_power_frequency = float(np.median(ridge[:reference_count]))
    shift = np.abs(ridge-low_power_frequency)
    frequency_step = float(np.median(np.diff(frequency)))
    shift_limit = max(1e6, 4*frequency_step)
    contrast_floor = max(.08, .65*float(np.median(contrast[:reference_count])))
    stable = (shift <= shift_limit) & (contrast >= contrast_floor)
    candidates = np.flatnonzero(stable)
    if not len(candidates):
        return {}, {"reliable": False, "reason": "no_stable_power_plateau"}
    chosen = int(candidates[-1])
    edge_hit = chosen in (0, len(powers)-1)
    checks = {"stable_plateau": bool(np.sum(stable) >= 3), "not_at_power_edge": not edge_hit,
              "notch_contrast": bool(contrast[chosen] >= contrast_floor),
              "frequency_sampling": bool(frequency_step <= shift_limit/4)}
    fit = {"readout_power_dbm": float(powers[chosen]), "readout_frequency_hz": float(ridge[chosen]),
           "low_power_frequency_hz": low_power_frequency, "nonlinear_shift_hz": float(shift[chosen]),
           "ridge_frequency_hz": ridge.tolist(), "contrast_by_power": contrast.tolist(),
           "power_edge_hit": edge_hit}
    quality = {"reliable": bool(all(checks.values())), "checks": checks,
               "reason": "measured_low_power_plateau_and_contrast", "uncertainty_method": "grid_resolution",
               "standard_errors": {"readout_power_dbm": float(np.median(np.diff(powers))),
                                   "readout_frequency_hz": frequency_step}}
    return fit, quality
...
def analyze(observation: dict) -> dict:
    ...
    if tool in ("sq.s21_power2d", "sq.s21_zpa2d"):
        if y.ndim != 2 or min(y.shape) < 16 or q.shape != y.shape or not np.isfinite(y).all() or not np.isfinite(q).all():
            raise ValueError("Invalid finite aligned 2D S21 I/Q grid")
        fitted, quality = (fit_s21_power2d(obs) if tool == "sq.s21_power2d" else fit_s21_zpa2d(obs))
