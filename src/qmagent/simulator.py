...
class AnalyticSimulator:
    ...
    def __init__(self, seed: int = 20260903, noise_scale: float = 1.0):
        ...
        self._truth = {
            ...
            "readout_optimum_dbm": self.rng.uniform(-24.0, -16.0),
    ...
    def measure(self, tool: str, state: dict, scan: dict) -> dict:
        ...
        elif tool == "sq.s21_power2d":
            self._power2d(obs, state, scan)
    ...
    def _power2d(self, obs: dict, state: dict, scan: dict) -> None:
        ...
        powers = np.linspace(scan.get("power_min_dbm", -45.), scan.get("power_max_dbm", -5.),
                             scan.get("power_points", 31))
        rows = []
        for power in powers:
            saturation = 1/(1+np.exp(-(power-self._truth["readout_optimum_dbm"]-5)/2.4))
            resonance = self._truth["fr"]+6e6*saturation
            signal = .5-.4*(1-.35*saturation)/(1+(2*(frequency-resonance)/2e6)**2)
            rows.append(signal+self._noise(len(frequency))+1j*self._noise(len(frequency)))
        grid = np.asarray(rows)
        obs["sweep"] = {"frequency_values_hz": frequency.tolist(), "power_values_dbm": powers.tolist(),
                        "shape": [len(powers), len(frequency)], "unit": ["dBm", "Hz"]}
        obs["measurement"] = {"i": grid.real.tolist(), "q": grid.imag.tolist(), "unit": "a.u."}
        from .analysis_tools import fit_s21_power2d
        obs["fit_result"], obs["quality"] = fit_s21_power2d(obs)
    ...
    def _iq(self, obs: dict, state: dict, scan: dict) -> None:
        ...
        power_gain = np.exp(-((state["readout_power_dbm"]-self._truth["readout_optimum_dbm"])/11.)**2)
        separation = 1.6 * readout_gain * amplitude_gain * power_gain
