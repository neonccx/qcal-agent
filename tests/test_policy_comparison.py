...
spec = importlib.util.spec_from_file_location("compare_policy", Path(__file__).resolve().parents[1]/"scripts/compare_policy.py")
...
class ComparisonTests(unittest.TestCase):
    def setUp(self):
        ...
        for directory, adapter, flags in ((self.before, None, [True, False]), (self.after, "adapter", [False, True])):
            directory.mkdir()
            (directory/"config.json").write_text(json.dumps(dict(protocol="1.0", model="same", test_sha256="frozen",
