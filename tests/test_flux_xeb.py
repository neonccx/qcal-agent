...
class CheckpointMigrationTests(unittest.TestCase):
    def test_v1_checkpoint_migrates_and_requires_new_stages(self):
        ...
        self.assertNotIn(2, restored.completed)
        self.assertNotIn(8, restored.completed)

    def test_v2_checkpoint_migrates_and_requires_power2d(self):
        runner = AgentRunner(PhysicalSimulator(33), RulePolicy())
        snapshot = runner.snapshot()
        snapshot["version"] = 2
        snapshot["state"].pop("readout_power_dbm")
        snapshot["completed"] = list(range(8))
        restored = AgentRunner(PhysicalSimulator(33), RulePolicy())
        restored.restore(copy.deepcopy(snapshot))
        self.assertEqual(restored.state["readout_power_dbm"], -30.0)
        self.assertNotIn(1, restored.completed)
        self.assertIn(8, restored.completed)
